...
\title{Android Collusion Detection Using Linear Temporal Logic}
\author{Richard Allen, Markus Roggenbach\\Swansea University, UK}
\date{MT-CPS 2021}
...
\frame{\frametitle{Intercept Operating System Calls in Android}
...
\item We theorise the number of computational steps follows $O(n(n+1)/2 * |\varphi|)$ where $n$ is the number of events in the trace and $\varphi$ is the size of the formula.
\end{itemize}
...
\item Comes at the expense that the LTL operators that deal with future events (next, eventually) cannot be implemented because we do not know what future events will be.  Operators that are concerned with past events are implemented (previous, once).
...
\item For comparison the eventually semantics:  $(\sigma, j) \models \diamond p\ iff\ (\sigma, k) \models p\ for\ some\ k \geq j$.
...
\frame{\frametitle{Runtime Monitoring In Context}

\begin{itemize}

\item Runtime monitoring has false positives (no checks for metadata), is effective. 

\item Model checking apps has false positives (over approximation), works for small apps only\footnote{Software Model Checking for Mobile Security(...).  Irina Mariuca Asavoae, Hoang Nga Nguyen, Markus Roggenbach.  SPIN 2018: 3-25}. 

\item Static analysis has false positives (no reachability analysis)\footnote{Detection of app collusion potential using logic programming.  Jorge Blasco, Thomas M. Chen, Igor Muttik, Markus Roggenbach.  J. Netw. Comput. Appli. 105: 88-104 (2018)}.  

\item Machine learning has false positive and false negatives\footnote{Towards a threat assessment framework for apps collusion.  Kalutarage, Harsha Kumara and Nguyen, Hoang Nga and Shaikh, Siraj Ahmed.  Telecommunication Systems.  Springer US 2017: 1-14}.
...
%Caveat: For properties involving sequence of multiple events and data, one needs to accept false positives or accept properties can be detected within a limited timeframe only.
...
\item Monitor for more properties, e.g. collusion over longer chains (not only 2 apps).
